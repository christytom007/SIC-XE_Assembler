...
{//To add 0's to Loc Eg: 0 -> 0000
	hexa temp = "";
	for (int i = 0; i<(dig - a.length()); ++i)//add 0's
		temp += '0';
	return temp + a;
}
...
{
	int i;
	for (i = 0; i<5; ++i) word[i] = "";//clearing the string
	count = 0;
	i = 0;
	while (isWhiteSpace(a[i]) && (i<(a.length())))//counting the white space
	{
		++i;
		//continue;
	}
	if (i == a.length() || a[i] == '.')   return;//checking for comments
	for (; i<a.length();)
	{//splitting the sentence
		while (isWhiteSpace(a[i]) && (i<a.length()))//skipping the white space
		{
			++i;
			//continue;
		}
		if (i == a.length()) break;//Is it end of the string?
		for (; !isWhiteSpace(a[i]) && (i<a.length()); ++i) word[count] += a[i];//Storing each word in word[count]
		++count;
	}
}
...
void run()
{
	create();//To create the mapping dirctory
	char ch;
	string s, word[5];
	int count = 0;
	fin1.open("C:/Assembler Files/Input.sic");
	fout1.open("C:/Assembler Files/intermediate.txt");
	error.open("C:/Assembler Files/error.txt");
	line = 5;//Initial line count
	getline(fin1, s);//First Line
	extract(s, word, count);//Extracting 1st line
	while (count == 0)
	...
	pc = "0";
	BLOCK["DEFAULT"].num = 0;//Initializing default block details
	...
	line = 5;
	if (word[0] == "START")
	{//Checking for start without label Eg:	START  0
		pc = word[1];
		fout1 << line << endl;
		fout1 << "" << endl;
		fout1 << "START" << endl;
	...
	{//checking for start with label Eg: FIBO    START  0
		pc = word[2];
		fout1 << line << endl;
		fout1 << word[0] << endl;
		fout1 << "START" << endl;
	...
	while (true)
	{
		...
		fout1 << line << endl;
		if (count == 0)
		{//checking for intermediate comments
	...
	hexa addr, len;
	string temp = find_block(0);//Getting address and length of DEFAULT BLOCK
	addr = BLOCK[temp].address;
	len = BLOCK[temp].length;
	for (int i = 1; i<block_num; ++i)
	{//To set address and length of each block
		temp = find_block(i);//Get the name of next block
		BLOCK[temp].address = toHex(toDec(addr) + toDec(len));//Increment address of 2nd block with 1st block
		addr = BLOCK[temp].address;//store the 2nd block address in address variable
		len = BLOCK[temp].length;//store the 2nd block length in length variable
	}
}
...
void execute(string word[], int count)
{
	...
	/*if (word[0] == "USE")
	{//For the begining of new blocks
	...
	if (word[0][0] == '+')
	{//Checking for Mnumonic type 4
		cout << "Format 4" << endl;
		fout1 << "" << endl;
		fout1 << word[0] << endl;
		fout1 << word[1] << endl;
		fout1 << pc << endl;
		pc = toHex(toDec(pc) + 4);//Incrementing pc by 4
		fout1 << pc << endl;
		return;
	}
	if (OPTAB[word[0]].exist == 'y')
	{//Checking for opcode
		cout <<OPTAB[word[0]].opcode<< " is opcode!" << endl;
		fout1 << "" << endl;
		fout1 << word[0] << endl;
		fout1 << word[1] << endl;
		fout1 << pc << endl;
		pc = toHex(toDec(pc) + OPTAB[word[0]].format);//Incrementing pc with the type of opcode
		fout1 << pc << endl;
		return;
	}
	if (OPTAB[word[0]].exist == 'n')
	{//If the dtring dose not exist in OPCODE
		if (SYMTAB[word[0]].exist == 'y')
		{//Checking for Repeated creation of labels
			error << "404 :Line " << line << ": Duplicate Symbol....." << endl;
			error_flag = 1;
		}
		else
		{//For creation of new Labels
			...
			fout1 << word[2] << endl;
			fout1 << pc << endl;
			if (word[1][0] == '+')//Checking for format 4 with label
				pc = toHex(toDec(pc) + 4);
			else if (OPTAB[word[1]].exist == 'y')//Incrementing PC with size of opcode
				pc = toHex(toDec(pc) + OPTAB[word[1]].format);
			else if (word[1] == "WORD")      pc = toHex(toDec(pc) + 3);//These are for Assembler dirctives
			else if (word[1] == "RESW")      pc = toHex(toDec(pc) + (atoi(word[2].c_str()) * 3));
			/* atoi(string converts sring to integer
			string.c_str() is used to convert string object to a real string
			also we can use,
			char *cstr=new char*[word[2].length()];
			strcpy(cstr,word[2].c_str());
			*/
			else if (word[1] == "RESB")      pc = toHex(toDec(pc) + atoi(word[2].c_str()));
			else if (word[1] == "BYTE")
			{//To count and reserve the string byte
				int len = word[1].length() - 3;
				if (word[1][0] == 'X') len /= 2;
				pc = toHex(toDec(pc) + len);
			}
			else
			{//if above cases did not execute and it is still here there is no opcode of the Mnumonic instruction
...
{
	ifstream fin;
	fin.open("C:/Assembler Files/modification.txt");
	string s;
	while (true)
	{
		getline(fin, s);
...
void input(string a[])
{//Read 6 lines of intermediate file and store in a[i]
	int i;
	for (i = 0; i<6; ++i)
		getline(intm, a[i]);
...
int main()
{
	...
	intm.open("C:/Assembler Files/intermediate.txt");
	obj.open("C:/Assembler Files/object.txt");
	lst.open("C:/Assembler Files/list.txt");
	mod.open("C:/Assembler Files/modification.txt");
	lst << "Line\tLoc   Block\t\tSource Statement\t\tObject Code" << endl;
	lst << "----------------------------------------------------------------" << endl << endl;
	input(a);//get 6 lines of code from intermediate file
	curr_block = "DEFAULT";
	curr_block_num = 0;
	while (a[1] == "$")
	{//Checking for comment until $ is not found
		lst << a[0] << "\t\t " << a[2] << endl;
		input(a);
	}
	if (a[2] == "START")
	{
		lst << a[0] << "\t" << extendTo(4, a[4]) << "  " << curr_block_num << "\t\t" << a[1] << "\t\t" << a[2] << "\t\t" << a[3] << endl;//Writing start to LIST
		obj << "H^";
		int i;
		for (i = 0; i<a[1].length(); ++i)//Writing program name to obj
			obj << a[1][i];
		for (; i<6; ++i)//Filling unused name space with whitespace
			obj << " ";
		string temp = find_block(block_num - 1);
		hexa len;
		len = toHex(toDec(BLOCK[temp].address) + toDec(BLOCK[temp].length));//Find the total length of program
		obj << "^" << extendTo(6, a[3]) << "^" << extendTo(6, len) << endl;
		start = a[3];//Set starting PC count
	}
	else
	{//Incase START is not found in intermediate file
		error_flag = 1;
	...
	while (true)
	{
		...
		// cout<<a[2]<<endl;
		assemble(a);
		cout << "opcode: " << a[2] << ":::"<<OPTAB[a[2]].opcode<<endl<<endl;
		// cin>>ch;
	}
	if (error_flag)
	{
...
void assemble(string a[])
{
	...
	int format;
	/*if (a[2] == "USE")
	{//Writing new Block datas to LIST
		curr_block = a[3];
...
string gen_code(string a[])
{
	...
	{//
		int i;
		ob1 = "";//Initializing
		if (a[3][0] == 'X')//Incase of byte Characters
		for (i = 2; i<a[3].length() - 1; ++i) ob1 += a[3][i];
		else //a[3][0]=='C'
		for (i = 2; i<a[3].length() - 1; ++i)//Incase of Byte Integers
			ob1 += toHex((int)a[3][i]);
		return ob1;
	}
	if (a[2] == "WORD")
	{//Count size of WORD in HEX
	...
	{//Incase of Format 4
		format = 4;
		a[2] = a[2].substr(1);//Replace + symbol with whitespace
	}
	else
		format = OPTAB[a[2]].format;//Incase of format other than 4
	if (format == 1)
	{
		cout << "Format 1" << endl;
	...
	if (format == 2)
	{
		...
		ob1 = OPTAB[a[2]].opcode;
		if (a[3].length() == 3)
		{//Checking for 2 register operation EG:	COMPR A,T
			ob2 = toHex(reg_num(a[3][0]));//For 1st Reg.
			if (isdigit(a[3][2]))  ob2 = ob2 + toHexDig(a[3][2] - 1);//isdigit(string) is used to check wether the string contain a digit
			else
			{//If it is a Reg rather than integer
		...
		{
			if (isdigit(a[3][0]))
			{//Incase there is only Integer
				ob2 = toHex(atoi(a[3].c_str())) + "0";//convert string obj ->string->integer
				cout << "isdigit! ob2: " << ob2 << endl;
			}
			else
			{//If only one Register
	...
	if (format == 3)
	{
		...
		{//Checking for Immediate!!
			imm = true;//setting immediate flag
			cout << "Immediate!" << endl;
			ob1 = toHex(toDec(ob1) + 1);//Increment ob1 by 1
			a[3] = a[3].substr(1);//Remove the # symbol
			if (isdigit(a[3][0]))
			{//Checking Immediate value is Digit
				ob2 = "0";
				ob3 = toHex(atoi(a[3].c_str()));//store integer in ob3
				return extendTo(2, ob1) + ob2 + extendTo(3, ob3);//Return total code
			}
			//cout<<"ob1: "<<ob1<<endl;
		}
		else if (a[3][0] == '@')
		{//If it is indirect
			ind = true;//Set indirect flag
			cout << "Indirect!" << endl;
			ob1 = toHex(toDec(ob1) + 2);
			a[3] = a[3].substr(1);//Remove @
		}
		else
